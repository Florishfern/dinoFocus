...
exports.processFocusEnd = async (req, res) => {
    try{
        ...
        if(!finalCategoryId && new_category_name){
            const [newCat] = await db.promise().execute(
                `INSERT INTO categories (name, user_id) VALUES (?, ?)`,
                [new_category_name, userId]
            );
        ...
        const [activePets] = await db.promise().execute(
        ...
        if(activePets.length > 0){
            ...
            await db.promise().execute(
                `UPDATE pets SET level = ?, exp = ? WHERE pets_id = ?`,
                [petResult.newLevel, petResult.newExp, currentPet.pets_id]
            )
        }
        ...
        if(!task_id && new_task_title){
            const [newTask] = await db.promise().execute(
                `INSERT INTO tasks (user_id, title, category_id, focus_time_spent, is_completed)
                VALUES (?, ?, ?, ?, 1)`,
                [userId, new_task_title, finalCategoryId, planned_minutes]
            );
            finalTaskId = newTask.insertId;
        } else if (task_id){
            await db.promise().execute(
                `UPDATE tasks SET is_completed = 1, category_id = ? WHERE task_id = ? AND user_id = ?`,
                [finalCategoryId || null, task_id, userId]
            );
        }
        ...
        res.status(200).json({
            success: true,
            message: petResult?.isLevelUp? "Level UP! Your pet is grow up": "Focus session ended",
            data: {
                task_id: finalTaskId,
